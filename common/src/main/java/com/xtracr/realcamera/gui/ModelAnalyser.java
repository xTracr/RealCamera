...
public class ModelAnalyser extends VertexRecorder {
    ...
    private static void drawPrimitive(GuiGraphics graphics, VertexData[] primitive, int argb, int offset) {
        ...
        for (VertexData vertex : primitive) buffer.vertex(vertex.x(), vertex.y(), vertex.z() + offset).color(argb).endVertex();
        if (primitive.length == 3) buffer.vertex(primitive[2].x(), primitive[2].y(), primitive[2].z() + offset).color(argb).endVertex();
        graphics.flush();
    }
    ...
    private static void drawNormal(GuiGraphics graphics, Vec3 start, Vec3 normal, int length, int argb) {
        ...
        buffer.vertex((float) start.x(), (float) start.y(), (float) (start.z() + 1200f)).color(argb).normal((float) normal.x(), (float) normal.y(), (float) normal.z()).endVertex();
        buffer.vertex((float) end.x(), (float) end.y(), (float) (end.z() + 1200f)).color(argb).normal((float) normal.x(), (float) normal.y(), (float) normal.z()).endVertex();
        graphics.flush();
    }
