...
public record ColoredFloatQuadRenderState(
    ...
    public void buildVertices(VertexConsumer vertexConsumer, float depth) {
        vertexConsumer.addVertexWith2DPose(pose, x0, y0, z0 + depth).setColor(argb);
        vertexConsumer.addVertexWith2DPose(pose, x1, y1, z1 + depth).setColor(argb);
        vertexConsumer.addVertexWith2DPose(pose, x2, y2, z2 + depth).setColor(argb);
        vertexConsumer.addVertexWith2DPose(pose, x3, y3, z3 + depth).setColor(argb);
    }
}
