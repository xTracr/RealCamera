...
public record ColoredFloatRectangleRenderState(
    ...
    public void buildVertices(VertexConsumer vertexConsumer, float depth) {
        vertexConsumer.addVertexWith2DPose(pose, x0, y0, z + depth).setColor(argb);
        vertexConsumer.addVertexWith2DPose(pose, x0, y1, z + depth).setColor(argb);
        vertexConsumer.addVertexWith2DPose(pose, x1, y1, z + depth).setColor(argb);
        vertexConsumer.addVertexWith2DPose(pose, x1, y0, z + depth).setColor(argb);
    }
}
