...
public class GUIHelper {
    ...
    public static void fill(GuiGraphics graphics, float x0, float y0, float x1, float y1, float z, int argb) {
        ((GuiGraphicAccessor) graphics).getGuiRenderState().submitGuiElement(new ColoredFloatRectangleRenderState(
                RenderPipelines.GUI, TextureSetup.noTexture(), new Matrix3x2f(graphics.pose()),
                x0, y0, x1, y1, z, argb, ((GuiGraphicAccessor) graphics).getScissorStack().peek()));
    }
    ...
    public static void renderPolygon(GuiGraphics graphics, VertexData[] polygon, float z, int argb) {
        ...
        ((GuiGraphicAccessor) graphics).getGuiRenderState().submitGuiElement(new ColoredFloatQuadRenderState(
                RenderPipelines.GUI, TextureSetup.noTexture(), new Matrix3x2f(graphics.pose()),
                x0, y0, z, x1, y1, z, x2, y2, z, x3, y3, z, argb, ((GuiGraphicAccessor) graphics).getScissorStack().peek()));
    }

    public static void renderVector(GuiGraphics graphics, Vec3 start, Vec3 vector, float z, int argb) {
        ((GuiGraphicAccessor) graphics).getGuiRenderState().submitGuiElement(new ColoredFloatLineRenderState(
                RenderPipelines.GUI, TextureSetup.noTexture(), new Matrix3x2f(graphics.pose()),
                (float) start.x(), (float) start.y(), z,
                (float) vector.x(), (float) vector.y(), 0, argb, ((GuiGraphicAccessor) graphics).getScissorStack().peek()));
    }
}
